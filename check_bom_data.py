...
try:
    ...
    if count > 0:
        ...
        # 按项目ID分组
        print('\n按项目ID分组:')
        cursor.execute("SELECT project_id, COUNT(*) FROM bom_table GROUP BY project_id ORDER BY project_id")
    ...
    if project_count > 0:
        # 检查所有项目
        print('\n所有项目:')
        cursor.execute("SELECT id, name, status, created_at FROM projects ORDER BY created_at DESC")
        projects = cursor.fetchall()
        for project in projects:
            print(f'ID: {project[0]}, 名称: {project[1]}, 状态: {project[2]}, 创建时间: {project[3]}')
