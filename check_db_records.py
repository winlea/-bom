...
def check_bom_records():
    ...
    try:
        ...
        # 查询所有BomTable记录
        cursor.execute("SELECT id, project_id, part_number, part_name, created_at FROM bom_table ORDER BY created_at DESC LIMIT 100")
        ...
        if records:
            ...
            for i, record in enumerate(records[:20]):
                ...
                print(f"{id} | {project_id} | {part_number} | {part_name} | {created_at}")
            
            # 按project_id分组统计
            cursor.execute("SELECT project_id, COUNT(*) FROM BomTable GROUP BY project_id")
        ...
        # 检查Project表
        cursor.execute("SELECT id, name, description, status, created_at FROM Project ORDER BY created_at DESC")
        ...
        # 检查ImportLog表
        cursor.execute("SELECT id, project_id, filename, created_count, errors_count, created_at FROM ImportLog ORDER BY created_at DESC")
        ...
        if import_logs:
            print("\n导入日志列表:")
            print("ID | Project ID | Filename | Created Count | Errors Count | Created At")
            print("-" * 100)
            
            for log in import_logs:
                id, project_id, filename, created_count, errors_count, created_at = log
                print(f"{id} | {project_id} | {filename} | {created_count} | {errors_count} | {created_at}")
        else:
            print("\n❌ 没有找到ImportLog记录")
