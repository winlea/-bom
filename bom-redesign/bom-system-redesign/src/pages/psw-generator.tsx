...
export default function PSWGeneratorPage() {
  ...
  const handleGeneratePSW = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://localhost:5000/api/psw/generate', {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        try {
          const errorData = await response.json();
          alert('生成PSW文件失败：' + (errorData?.details || errorData?.error || response.status));
        } catch (e) {
          alert('生成PSW文件失败：' + response.status);
        }
      }
    } catch (error: any) {
      console.error('生成PSW错误:', error);
      alert('生成PSW文件时发生错误：' + (error?.message || error));
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-2xl font-bold text-gray-900">PSW零件保证书生成器</h1>
            </div>
          </div>
        </div>
      </div>
