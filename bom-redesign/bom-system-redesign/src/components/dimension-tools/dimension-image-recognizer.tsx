...
export default function DimensionImageRecognizer({ onDimensionsRecognized, onCancel }: DimensionImageRecognizerProps) {
  ...
  const handleSaveEdit = () => {
    ...
    setRecognizedDimensions((prev) => prev.map((dim) => (dim.id === editingDimension ? { ...dim, ...editForm } : dim)));
    setEditingDimension(null);
    setEditForm({});
  };
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          {imagePreview && !isRecognized && (
            <Button onClick={simulateRecognition} disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-700">
              {isLoading ? (
                <>
          ...
          {isRecognized && (
            <div className="space-y-4">
              <Alert variant="default">
                ...
                <AlertDescription>成功识别到 {recognizedDimensions.length} 个尺寸</AlertDescription>
              </Alert>
                      ...
                      <tr key={dim.id} className={index % 2 === 0 ? '' : 'bg-slate-50'}>
                        {editingDimension === dim.id ? (
                          ...
                          <>
                            ...
                            <td className="px-4 py-2">
                              <div className="flex gap-2">
                                <Button size="sm" onClick={() => handleEdit(dim)} variant="outline">
                                  <Edit className="h-3 w-3 mr-1" />
                                  编辑
                                </Button>
                                <Button size="sm" onClick={() => handleDeleteDimension(dim.id)} variant="destructive">
