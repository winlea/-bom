    ...
    def validate_template_fill(self, template_file, part_data):
        ...
        try:
            ...
            placeholders = [
                ...
                "{{MATERIAL_SPEC}}", "{{PRODUCT_NAME}}", "{{DRAWING_2D}}",
                "{{DRAWING_3D}}", "{{MATERIAL_THICKNESS}}"
            ]
            ...
            for idx, size_data in enumerate(part_data['drawing_sizes']):
                ...
                if row <= size_area_end:
                    ...
                    if ws.cell(row=row, column=size_col):
                        ...
                        ws.cell(row=row, column=tolerance_col, value=size_data['tolerance'])
        ...
        for template in template_files:
            for part in test_parts:
                ...
                if result['status'] == 'success':
                    print(f"   ✅ 成功: 填充了 {result['filled_placeholders']} 个占位符")
                else:
