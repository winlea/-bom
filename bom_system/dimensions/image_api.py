...
def save_canvas_image():
    ...
    try:
        ...
        if success:
            return jsonify({"success": True, "message": "Canvas图片保存成功"})
        else:
            return jsonify({"success": False, "message": "Canvas图片保存失败"}), 500

    except Exception as e:
        logger.error(f"保存Canvas图片失败: {str(e)}")
        return (
            jsonify({"success": False, "message": f"保存Canvas图片失败: {str(e)}"}),
            500,
        )
...
def save_combined_images():
    ...
    try:
        ...
        return jsonify(
            {
                "success": True,
                "message": "拼接图片保存成功",
                "data": {"combinedImageUrl": combined_image_url},
            }
        )
        ...
        return (
            jsonify({"success": False, "message": f"保存拼接图片失败: {str(e)}"}),
            500,
        )
...
def batch_save_canvas_images():
    ...
    try:
        ...
        if not canvas_images:
            ...
            return (
                jsonify({"success": False, "message": "请提供Canvas图片数据列表"}),
                400,
            )
        ...
        try:
            ...
            return (
                jsonify({"success": False, "message": f"数据库连接失败: {str(e)}"}),
                500,
            )
        ...
        for i, img_data in enumerate(canvas_images):
            try:
                ...
                logger.info(
                    f"Canvas数据长度: {len(canvas_data_url) if canvas_data_url else 0}"
                )
...
def get_dimension_image(dimension_id):
    ...
    try:
        ...
        if image_data:
            ...
            return jsonify({"success": False, "message": "未找到图片数据"}), 404

    except Exception as e:
        logger.error(f"获取尺寸图片失败: {str(e)}")
        return (
            jsonify({"success": False, "message": f"获取尺寸图片失败: {str(e)}"}),
            500,
        )
...
def delete_dimension_image(dimension_id):
    ...
    try:
        ...
        if success:
            ...
            return jsonify({"success": False, "message": "图片删除失败或不存在"}), 404

    except Exception as e:
        logger.error(f"删除尺寸图片失败: {str(e)}")
        return (
            jsonify({"success": False, "message": f"删除尺寸图片失败: {str(e)}"}),
            500,
        )


@dimension_image_bp.route(
    "/project/<project_id>/part/<part_id>/combined", methods=["POST"]
)
def create_combined_dimension_sheet():
    ...
    try:
        ...
        # 获取该零件的所有尺寸
        dimensions = dimension_service.get_dimensions_by_part_number(
            project_id, part_id
        )

        if not dimensions:
            db_session.close()
            return jsonify({"success": False, "message": "未找到该零件的尺寸数据"}), 404
        ...
        if not dimension_images:
            db_session.close()
            return (
                jsonify({"success": False, "message": "该零件没有可拼接的尺寸图片"}),
                400,
            )
        ...
        return (
            jsonify({"success": False, "message": f"创建尺寸拼接表失败: {str(e)}"}),
            500,
        )
