...
class DimensionImageService:
    ...
    def save_canvas_image_to_db(
        ...
        try:
            ...
            filename = f"dimension_{dimension_id}_{image_type}_{uuid.uuid4().hex[:8]}.png"
    ...
    def save_combined_dimension_images(
        ...
        try:
            ...
            # 创建拼接图片
            combined_image = self._create_combined_image(dimension_images, layout_config)
    ...
    def _create_combined_image(self, dimension_images: list, layout_config: dict = None) -> Image.Image:
        """
        创建拼接图片
        ...
        try:
            ...
            combined_image = Image.new('RGB', (canvas_width, canvas_height), background_color)
            ...
            # 计算每个图片的可用空间
            available_width = (canvas_width - 2 * padding - (grid_cols - 1) * spacing) // grid_cols
            available_height = 100  # 每行高度
            
            current_row = 0
            current_col = 0
            
            for img_data in dimension_images:
                try:
                    # 解析图片数据
                    ...
                    # 计算粘贴位置
                    x = padding + current_col * (available_width + spacing)
                    y = padding + current_row * (available_height + spacing)
