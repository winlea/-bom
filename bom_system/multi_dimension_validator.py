...
class MultiDimensionValidator:
    ...
    def create_test_part_with_multiple_dimensions(self, part_number, dimensions_count=5):
        """创建包含多个尺寸的测试零件"""
        dimensions = []
    ...
    def create_template_with_multiple_dimensions(self, part_data, template_name):
        ...
        for idx, dim in enumerate(part_data['dimensions']):
            ...
            # 设置样式
            for col in ['B', 'C', 'D', 'E']:
                ws[f'{col}{row}'].fill = PatternFill(start_color="E6F3FF", end_color="E6F3FF", fill_type="solid")
    ...
    def validate_dimension_placement(self, template_path, expected_count):
        ...
        try:
            ...
            return {
                ...
                'placement_check': 'B12区域放置正确' if actual_count == expected_count else 'B12区域放置不正确'
            }
    ...
    def run_multi_dimension_test(self):
        ...
        for case in test_cases:
            print(f"测试零件 {case['part_number']} 的 {case['dimensions_count']} 个尺寸...")
            ...
            template_path = self.create_template_with_multiple_dimensions(test_part, template_name)
            ...
            print(f"{status} 零件 {case['part_number']} - 找到 {validation_result['actual_count']} 个尺寸")
    ...
    def generate_test_report(self):
        ...
        report = {
            ...
            'passed_tests': sum(1 for r in self.test_results if r['validation']['success']),
            'failed_tests': sum(1 for r in self.test_results if not r['validation']['success']),
        ...
        if report['failed_tests'] > 0:
            ...
            for result in self.test_results:
                if not result['validation']['success']:
                    print(f"- 零件 {result['part_number']}: {result['validation']['placement_check']}")
