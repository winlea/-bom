...
class BOMRepository(IBOMRepository):
    ...
    def get_by_part_number(self, part_number: str) -> Optional[Any]:
        """根据零件编号获取BOM记录"""
        return self.session.query(BomTable).filter(BomTable.part_number == part_number).first()
    
    def get_by_project(self, project_id: int) -> List[Any]:
        """根据项目ID获取BOM记录"""
        return self.session.query(BomTable).filter(BomTable.project_id == project_id).all()
    
    def get_by_sequence(self, sequence: str) -> Optional[Any]:
        """根据序号获取BOM记录"""
        return self.session.query(BomTable).filter(BomTable.sequence == sequence).first()
    
    def search(self, query: str, limit: int = 20) -> List[Any]:
        """搜索BOM记录"""
        like = f"%{query}%"
        return self.session.query(BomTable).filter(
            BomTable.part_number.like(like)
        ).order_by(BomTable.created_at.desc()).limit(limit).all()
