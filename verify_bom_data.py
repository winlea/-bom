...
def verify_bom_data():
    ...
    with app.app_context():
        ...
        if total_records > 0:
            print("\n最近10条记录:")
            recent_records = BomTable.query.order_by(BomTable.created_at.desc()).limit(10).all()
            ...
            # 检查是否有图片数据
            has_image_data = BomTable.query.filter(BomTable.image_data.isnot(None)).count()
            print(f"\n包含图片数据的记录数: {has_image_data}")
            
            # 检查是否有URL图片
            has_image_url = BomTable.query.filter(BomTable.image_url.isnot(None)).count()
