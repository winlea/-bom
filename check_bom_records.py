...
from bom_system.models import BomTable, ImportLog, Project, db
...
def check_bom_records():
    ...
    with app.app_context():
        ...
        for i, bom in enumerate(recent_bom_records, 1):
            print(
                f"{i}. ID: {bom.id}, Project ID: {bom.project_id}, Part Number: {bom.part_number}"
            )
        print()
        ...
        print("最近5条导入日志:")
        recent_import_logs = (
            ImportLog.query.order_by(ImportLog.id.desc()).limit(5).all()
        )
        for i, log in enumerate(recent_import_logs, 1):
            print(
                f"{i}. ID: {log.id}, Project ID: {log.project_id}, Created Count: {log.created_count}, Errors Count: {log.errors_count}"
            )
        print()
